...
<ul class="ss cart_list product_list_widget <?php echo $args['list_class']; ?>">
        ...
        foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
            ...
            if ($_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key)) {
                ...
                <li class="<?php echo esc_attr(apply_filters('woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key)); ?>">
                    ...
                    <?php
//                    $string = preg_replace("/<span[^>]+\>/i", "", $product_price);
//                    $string = substr($string, 5);
//                    $str_arr = explode(".", $string);
                    $amount = preg_replace( '#[^\d.]#', '', WC()->cart->get_product_price($_product));
                    $amount = intval(substr($amount, 2));
                    $sub_total = $sub_total + ($amount * $cart_item['quantity']);
                    ?>
...
<?php if (!WC()->cart->is_empty()) : ?>
    ...
    <p class="total"><strong><?php _e('Subtotal', 'woocommerce'); ?>:</strong> $<?php echo $sub_total; ?></p>
