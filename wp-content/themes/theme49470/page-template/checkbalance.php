...
if (isset($_POST['card_number']) && $_POST['card_number'] !="") {
	if(isset($_POST['ff_check_balance'])){
...
//			$givex_response = "cURL Error #:" .curl_errno($curl). $err;
	...
	if(isset($_POST['ff_get_history'])){
		...
		if( !empty($givex_number) ) {
			...
			if ($err) {
				$history_response = "Something went wrong";
			} else {
				$result_array = json_decode($response);
				$result = $result_array->result;
				if($result['1']==0){
					$history_response = '<h2>Your card balance is <span style="color:#FC3030 ">$'.$result['2'].'</span></h2>';
					$history_response .= '<h3>Transaction History:</h3>';
					$history_response .=  '<table class="table table-bordered">
                    <tr>
						<th width="20%">Type</th>
					...
					foreach($result['5'] as $res){
						$type = $res['2'];
						$store = $res['4'];
						if($type=="Activate"){
							$store = "Food Fight";
						}
						$history_response .=  '<tr><td>'. $type. '</td><td>'. $res['3']. '</td><td>'. $res['0']. '</td><td>'. $res['1']. '</td><td>'. $store. '</td></tr>';
					}
					$history_response .='</table>';
				}else{
					$history_response = "<span style='color:#FC3030'>".$result['2']."</span>";
				}
			}
			curl_close($curl);
		}
					...
					<form id="checkbalance" method="post" action="">
						...
						<div>
							<div>
								...
								<!--				                Check Balance-->
								<!--				            </button>-->
								<button type="submit" class="btn btn-default" id="ff_get_history" name="ff_get_history">
									Check Balance
								</button>
								<?php
								if(isset($givex_response)){
									echo "<span style='color:#FC3030'>".$givex_response."</span>";
								}?>
							</div>
						</div>
					</form>
