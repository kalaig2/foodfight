    ...
    if (has_term('gift-cards', 'product_cat')) {
        ...
        echo '<div class="gc-prices">';
            ...
            while (have_rows('price_options')) {
                ...
                echo '<input type="button" class="amt-but" name="gc-prices" id="price-' . $i . '" value="$ ' . $gc_price . '" >';
                $i++;
            }
        ...
        /*echo '<div id="custom-price">';
//            echo "<br>Enter the custom amount";
            echo "<input type='button' name='other' id='other' value='Other'>";
            echo "<input type='text' name='custom-price' id='custom-price' class='custom-price'>";
        echo "</div>";*/
...
function filter_woocommerce_cart_no_shipping_available_html( $p____there_are_no_shipping_methods_available_please_double_check_your_address_or_contact_us_if_you_need_any_help_woocommerce_p ) {
    ...
    foreach($items as $item) {
        $slug = find_cat_slug($item['product_id']);
        if( $slug == 'gift-cards' ) {
            $new_msg = "Shipping is not available for this product";
        } else {
