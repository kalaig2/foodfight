...
if (!function_exists('custom_posts_grid_shortcode')) {
	...
	function custom_posts_grid_shortcode($atts, $content = null) {
		extract(shortcode_atts(array(
			'type'            => 'post',
			'category'        => '',
			'custom_category' => '',
...
$output .= '<div class="div_thumbnail">';
						if ( $images ) {
					...
					$output .= '<time datetime="'.get_the_time('Y-m-d\TH:i:s', $post_id).'">' .get_the_date('d.m.Y'). '</time>';
					$output .= '</span>';
$output .= '</div>';
